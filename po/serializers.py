...
class ItemSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        instance = self.Meta.model.objects.create(description=description,
                                                  purchase_order=purchase_order,
                                                  unit_cost=unit_cost,
                                                  discount=discount, **validated_data)
    ...
    def _change_supply_cost(self, supply, cost):
        ...
        This will change the supply's product cost, respective of supplier,
        in the database and will log the event as 'PRICE CHANGE'
        """
        try:
    ...
    def _log_quantity_change(self, obj, old_quantity, new_quantity, employee=None):
        ...
        if new_quantity != old_quantity:
            ...
            message = u"{0}ed {1}{2} {3} {4}"
            log = Log(supply=obj,
                      action=action,
                      quantity=diff,
                      employee=employee,
                      message=message.format(action.capitalize(),
                                             diff,
                                             obj.units,
...
class PurchaseOrderSerializer(serializers.ModelSerializer):
    ...
    project = serializers.PrimaryKeyRelatedField(required=False, allow_null=True,
    ...
    def create(self, validated_data):
        """
        Override the 'create' method to customize how items are created
        and pass the supplier instance to the item serializer via context
        """
        ...
        # Extract discount and terms from supplier of the purchase order
        discount = validated_data.pop('discount', validated_data['supplier'].discount)
        terms = validated_data.pop('terms', validated_data['supplier'].terms)
        
        # instance creation and attribute setting
        ...
        item_serializer_context = {'supplier': instance.supplier, 'po': instance}
        item_serializer = ItemSerializer(data=items_data,
                                         context=item_serializer_context,
                                         many=True)
    ...
    def update(self, instance, validated_data):
        """
        Override the 'update' method in order to increase the revision number and
    ...
    def receive_order(self, instance, validated_data):
        """
        Will received the order and then process the items and
        the corresponding supplies. The quantities for the supplies
        will automatically increase based on the supplies received
        """
        ...
        for item_data in items_data:
            try:
                ...
                serializer = ItemSerializer(item,
                                            context={'supplier': instance.supplier},
                                            data=item_data)
    ...
    def _log_receiving_item(self, item):
        ...
        message = u"Received {0}{1} of {2} from {3}"
        log = Log(supply=item.supply,
                  supplier=item.purchase_order.supplier,
                  action="ADD",
                  quantity=item.quantity,
                  message=message.format(item.quantity,
    ...
    def _email_purchaser(self, purchase_order):
        ...
        if purchase_order.employee.email:
            ...
            subject = u'Purchase Order from {0} Received'.format(supplier_name)
